...
<!-- just make the first post look different -->
<?php elseif( !$paged && $c == 1) :?>
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<div id="cover-img-container"><?php	the_post_thumbnail('orginal');	?></div>
		<div id="cover-text">
			<h2 class="cover-entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'boilerplate' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
			<?php the_excerpt(); ?>
		</div><!-- cover-text -->
	</article>
...
<?php else : ?>
	...
	<?php else : ?>
			<div class="entry-content">
				...
				<div class="posted-on"><?php boilerplate_posted_on(); ?> in 	<?php if ( count( get_the_category() ) ) : ?>
						<?php printf( __( '%2$s', 'boilerplate' ), 'entry-utility-prep entry-utility-prep-cat-links', get_the_category_list( ' ' ) ); ?></div>
	...
	<?php endif; ?>
			...
			<footer class="entry-utility">
				...
				<span class="share-tools">
					<div href="<?php the_permalink() ?>" class="fb-like" data-send="true" data-layout="button_count" data-width="" data-show-faces="true"></div>
