...
if ( !class_exists( 'TribeCommunityEvents' ) ) {
	...
	class TribeCommunityEvents {
		...
		public function sendEmailAlerts( $tribe_event_id ) {
			...
			ob_start();
			include TribeEventsTemplates::getTemplateHierarchy( 'community/email-template' );
		...
		public function doEventForm( $id = null ) {
			...
			if ( $this->allowAnonymousSubmissions || is_user_logged_in() ) {
				...
				if ( !empty($submission) ) {
					require_once('Submission_Handler.php');
				...
				if ( $show_form ) {
					...
					if ( empty($submission) || $this->messageType == 'error' ) {
						...
						$form = new Tribe__Events__Community__Event_Form( $event, $required, $errors );
						$output .= $form->render();
					} else {
						ob_start();
		 ...
		 * @see TribeCommunityEvents::doEventForm()
		 * @param int $event_id
		 * @return void
		 */
		...
		public function get_view_edit_links( $event_id ) {
			...
			if ( current_user_can('edit_post', $event_id) ) {
				$edit_link = sprintf('<a href="%s" class="edit-event">%s</a>', tribe_community_events_edit_event_link($event_id), __( 'Edit', 'tribe-events-community' ) );
			}
		...
		public function doVenueForm( $tribe_venue_id ) {
			...
			if ( ( isset( $_POST[ 'community-event' ] ) && $_POST[ 'community-event' ] ) && check_admin_referer( 'ecp_venue_submission' ) ) {
				if ( isset( $_POST[ 'post_title' ] ) && $_POST[ 'post_title' ] ) {
					...
					require_once( 'Venue_Submission_Scrubber.php' );
		...
		public function doOrganizerForm( $tribe_organizer_id ) {
			...
			if ( ( isset( $_POST[ 'community-event' ] ) && $_POST[ 'community-event' ] ) && check_admin_referer( 'ecp_organizer_submission' ) ) {
				if ( isset( $_POST[ 'post_title' ] ) && $_POST[ 'post_title' ] ) {
					...
					require_once( 'Submission_Scrubber.php' );
					require_once( 'Organizer_Submission_Scrubber.php' );
		...
		public function load_captcha_plugin() {
			require_once( dirname( __FILE__ ) . '/Captcha/Recaptcha.php' );
			$this->captcha = apply_filters( 'tribe_community_events_captcha_plugin', new Tribe__Events__Community__Captcha__Recaptcha() );
			if ( empty( $this->captcha ) ) {
				require_once( dirname( __FILE__ ) . '/Captcha/Null_Captcha.php' );
